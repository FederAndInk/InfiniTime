...
constexpr std::size_t Pinetime::Controllers::CalendarEventService::CalendarEvent::maxTitleSize;

Pinetime::Controllers::CalendarEventService::CalendarEventService()
  : characteristicDefinition {{{.uuid = &calEventAddCharUuid.u,
                                .access_cb = [](uint16_t conn_handle, uint16_t attr_handle, ble_gatt_access_ctxt* ctxt, void* arg) -> int {
...
void Pinetime::Controllers::CalendarEventService::Init() {
...
int Pinetime::Controllers::CalendarEventService::OnCommand(uint16_t conn_handle, uint16_t attr_handle, ble_gatt_access_ctxt* ctxt) {
  if (ctxt->op == BLE_GATT_ACCESS_OP_WRITE_CHR) {
    ...
    CalendarEvent* newEv = [&]() -> CalendarEvent* {
      if (calEvents.capacity() == calEvents.size()) {
        ...
        return static_cast<CalendarEvent*>(calEvents.emplace_back());
      }
    }();
    if (newEv) {
